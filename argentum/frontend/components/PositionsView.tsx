...
export default function PositionsView({ initial }: { initial: PositionsResponse }) {
  ...
  const handleClose = async (id: string) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.realized_pnl_pct > 0) setConfetti(true);
  ...
  const handleCloseAll = async () => {
    const profitableOnly = data.positions.filter(
      (p) => (p.market_pnl_pct ?? p.unrealized_pnl_pct) > 0,
    );
    const choice = window.prompt(
      `Закрыть позиции:\n` +
      `1 = ВСЕ (${data.positions.length})\n` +
      `2 = только прибыльные (${profitableOnly.length})\n` +
      `Введи 1 или 2, отмена — пусто`,
      "",
    );
    if (choice !== "1" && choice !== "2") return;
    const toClose = choice === "1" ? data.positions : profitableOnly;
    ...
    for (const p of toClose) {
      try {
        ...
        if (res.success && res.realized_pnl_pct > 0) anyProfit = true;
      } catch {}
    }
    if (anyProfit) setConfetti(true);
    setTimeout(() => setConfetti(false), 100);
    showToast("ok", `Закрыто: ${toClose.length}`);
    setClosingAll(false);
    await refresh();
  };
          ...
          <div className="flex items-baseline justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={closingAll || data.positions.length === 0}
                className="text-xs px-3 py-1.5 rounded-md border border-[var(--border)] hover:bg-[var(--bg-subtle)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors disabled:opacity-50"
                title="Закрыть все или только прибыльные"
              >
                {closingAll ? "Закрываем…" : "Закрыть несколько…"}
              </button>
            </div>
          </div>
          <SandboxDisclaimer
            hasSynced={data.positions.some((p) => p.source === "tinkoff_sync")}
          />
          <AnimatePresence>
    ...
    >
      <div className="flex items-start justify-between gap-6 flex-wrap">
        ...
        <motion.button
          ...
          onClick={onOpen}
          disabled={busy}
          className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl border transition-colors disabled:opacity-50 ${
            data.can_buy
              ? "border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 text-sm font-medium"
              : "border-[var(--border-soft)] bg-transparent hover:bg-[var(--bg-subtle)]/50 text-[var(--text-muted)] text-xs font-normal"
          }`}
          title={data.can_buy ? "" : "Модель не рекомендует, но можно открыть вручную"}
        >
          {busy ? (
            ...
            <><Plus className="h-3.5 w-3.5" />Открыть вручную (не рек.)</>
          )}
        </motion.button>
      </div>
    </motion.section>
...
function PositionCard({
  ...
  // Используем market P&L (реальный рынок без sandbox slippage) как основной
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          <div className="flex items-baseline gap-4 flex-wrap font-[family-name:var(--font-mono)] tabular-nums">
            ...
            <div title="P&L по реальной цене серебра (live market, без sandbox slippage)">
              ...
              <CountUp
                value={effectivePnl * 100}
                decimals={2}
                duration={500}
                prefix={effectivePnl > 0 ? "+" : ""}
                suffix="%"
          ...
          <div className="text-xs text-[var(--text-muted)] font-[family-name:var(--font-mono)]">
            {position.advice_reason.replace(
              /P&L\s+([-+]?\d+\.\d+%)/,
          ...
          <div className="flex items-center gap-4 text-[11px] text-[var(--text-faint)] font-[family-name:var(--font-mono)] flex-wrap">
            {position.source === "tinkoff_sync" ? (
              <span className="text-amber-400/60">
                импортировано {position.opened_at.slice(0, 10)} · реальная дата входа неизвестна (только avg_price из Tinkoff)
              </span>
            ) : (
...
function SandboxDisclaimer({ hasSynced }: { hasSynced: boolean }) {
  ...
  return (
    <div className="rounded-xl border border-amber-500/15 bg-amber-500/[0.03] px-4 py-2.5 flex items-start gap-2">
      ...
      <p className="text-xs text-[var(--text-muted)] leading-relaxed">
        <span className="text-amber-400/80 font-medium">📥 Импортированные позиции:</span>{" "}
        P&L накопленный за всё время удержания контрактов (могут быть куплены
        раньше через paper-trading). <b>Модель НЕ открывала эти сделки сегодня</b> —
        это снимок твоего фактического Tinkoff portfolio.
      </p>
    </div>
  );
}
