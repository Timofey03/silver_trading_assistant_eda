...
if not trades.empty:
    ...
    # Текущий live-сигнал (только для E3b — V25 показывается отдельно)
    live_sig_for_chart = sig if chart_model.startswith("🏆") else None

    fig = equity_curve(trades, bnh, strategy_name=strategy_name,
                       show_buy_sell_markers=True, tinkoff_orders=tinkoff_orders,
                       current_signal=live_sig_for_chart,
                       show_range_selector=True)
    st.plotly_chart(fig, use_container_width=True)

    # Объяснение под графиком — что показано
    with st.expander("ℹ️ Что показано на графике? Как это связано с реальной торговлей?"):
        st.markdown("""
**Это график бэктеста (исторической симуляции), не реальный портфель.**

| Элемент | Что значит |
|---|---|
| 🟦 Зелёная линия | Equity исторической стратегии — «что было бы, если бы помощник торговал на исторических данных» |
| 💛 Жёлтый пунктир | Buy & Hold силера за тот же период (для сравнения) |
| 🟢 BUY (треугольники вверх) | Момент входа в позицию в бэктесте |
| 🔴 SELL (треугольники вниз) | Момент выхода из позиции в бэктесте (наведите для деталей) |
| 💎 Tinkoff diamonds | Реальные paper-trading ордера через Tinkoff sandbox (если есть) |
| 🎯 Звезда «Сегодня» | Live сигнал на текущий момент (генерируется ежедневно через `scripts/daily_e3b.py`) |
| ⏸ Серый пунктир «Конец бэктеста» | Где заканчиваются исторические backtest-сделки |

**Архитектура помощника:**
1. **Бэктест** (исторические симуляции) — модель торгует в Python-симуляторе на 10+ лет истории
2. **Live сигнал** — каждый день 3× генерируется новый BUY/HOLD/SELL для сегодняшней цены
3. **Tinkoff sandbox** (опционально) — отдельный скрипт читает сигнал и кладёт ордер в sandbox

Помощник **не торгует автоматически** реальными деньгами — он только генерирует сигналы.
Решение покупать/продавать остаётся за пользователем.

**Почему последняя точка backtest — SELL?**
Каждая сделка в бэктесте имеет вход (BUY) и выход (SELL).
Последняя завершённая сделка — это её SELL точка. Это не сегодня —
для сегодняшнего сигнала смотрите ⭐ звезду или верхнюю карточку «Главная».
        """)
