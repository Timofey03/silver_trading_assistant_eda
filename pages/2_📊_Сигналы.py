...
# Badge для ясности что это backtest, не реальные ордера
st.info(
    "📚 **Это исторические предсказания ML-модели на бэктесте.** "
    "Каждая строка = что модель предсказала на этот день из исторических данных. "
    "Реально исполненные ордера в Tinkoff sandbox → страница **💰 Портфель → История ордеров**."
)
